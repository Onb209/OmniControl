...
def main():
    ...
    upper_body_skeleton = paramUtil.upperbody_kinematic_chain
    sample_files = []
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            # breakpoint()
            plot_3d_motion(animation_save_path, skeleton, upper_body_skeleton, motion, dataset=args.dataset, title=caption, fps=fps, hint=hint)
...
def generate_upper_body_fixed_sequences():
    args = generate_args()
    fixseed(args.seed)
    out_path = args.output_dir
    name = os.path.basename(os.path.dirname(args.model_path))
    ...
    #Load Input motion and normalize it.
    # input_motions_double = np.load('dataset/new_joints_vec/userstudy_0_converted.npy', allow_pickle=True)
    input_motions_double = np.load(args.input_motion, allow_pickle=True)
    input_motions_double = torch.from_numpy(input_motions_double).unsqueeze(0).unsqueeze(-1) # [1, 263, 1, frames]
    input_motions = input_motions_double.to(torch.float32)
    input_motions = input_motions.permute(0, 2, 3, 1) # [1, 263, 1, frames]
    if input_motions.shape[3] > n_frames:
        input_motions = input_motions[:, :, :, :n_frames]
    
    spatial_norm_path = './dataset/humanml_spatial_norm'
    mean = torch.from_numpy(data.dataset.t2m_dataset.mean).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
    std = torch.from_numpy(data.dataset.t2m_dataset.std).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
    input_motions = (input_motions - mean) / std
    input_motions = input_motions.to(dist_util.dev())
    
    # add inpainting mask according to args
    gt_frames_per_sample = {}
    if args.edit_mode == 'upper_body':
    ...
    for rep_i in range(args.num_repetitions):
        ...
        all_motions.append(sample.cpu().numpy().copy())

        original_motion = model_kwargs['y']['inpainted_motion']
        if model.data_rep == 'hml_vec':
            n_joints = 22 if original_motion.shape[1] == 263 else 21
            original_motion = data.dataset.t2m_dataset.inv_transform(original_motion.cpu().permute(0, 2, 3, 1)).float()
            original_motion = recover_from_ric(original_motion, n_joints)
            original_motion = original_motion.view(-1, *original_motion.shape[2:]).permute(0, 2, 3, 1)

        rot2xyz_pose_rep = 'xyz' if model.data_rep in ['xyz', 'hml_vec'] else model.data_rep
        rot2xyz_mask = None if rot2xyz_pose_rep == 'xyz' else model_kwargs['y']['mask'].reshape(args.batch_size, n_frames).bool()
        original_motion = model.rot2xyz(x=original_motion, mask=rot2xyz_mask, pose_rep=rot2xyz_pose_rep, glob=True, translation=True,
                               jointstype='smpl', vertstrans=True, betas=None, beta=0, glob_rot=None,
                               get_rotations_back=False)
        original_motions.append(original_motion.cpu().numpy().copy())
        all_lengths.append(model_kwargs['y']['lengths'].cpu().numpy())

        print(f"created {len(all_motions) * args.batch_size} samples")

    original_motion = np.concatenate(original_motions, axis=0)
    all_motions = np.concatenate(all_motions, axis=0)
    all_motions = all_motions[:total_num_samples]  # [bs, njoints, 6, seqlen]
    all_text = all_text[:total_num_samples]
    all_lengths = np.concatenate(all_lengths, axis=0)[:total_num_samples]
    ...
    skeleton = paramUtil.kit_kinematic_chain if args.dataset == 'kit' else paramUtil.t2m_kinematic_chain
    animation_save_path = os.path.join(out_path, "original_motion.mp4")
    plot_3d_motion(animation_save_path, skeleton, original_motion[0].transpose(2, 0, 1), dataset=args.dataset, title="original_motion", fps=fps, hint=None)
    input_npy_path = os.path.join(out_path, 'original_motion.npy')
    print(f"saving original motions to [{input_npy_path}]")
    np.save(input_npy_path,
            {'motion': original_motion, 'text': all_text, 'lengths': all_lengths, "hint": all_hint_for_vis,
             'num_samples': args.num_samples, 'num_repetitions': args.num_repetitions})
