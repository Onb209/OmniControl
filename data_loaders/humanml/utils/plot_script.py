...
def plot_3d_motion(save_path, kinematic_tree, hint_kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
    ...
    def update(index):
        ...
        for i, (chain, color) in enumerate(zip(kinematic_tree, used_colors)):
            ...
            ax.plot3D(data[index, chain, 0], data[index, chain, 1], data[index, chain, 2], linewidth=linewidth,
                      color=color)
        if hint is not None:
            for j, (chain_u, color_u) in enumerate(zip(hint_kinematic_tree, used_colors_upper)):
                if j < 5:
                    linewidth = 4.0
                else:
                    linewidth = 2.0
                # breakpoint()
                ax.plot3D(hint[index, chain_u, 0] - trajec[index, 0], hint[index, chain_u, 1], hint[index, chain_u, 2]- trajec[index, 1], linewidth=linewidth,
                        color=color_u)
